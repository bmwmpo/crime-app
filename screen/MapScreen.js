...
const MapScreen = () => {
  ...
  const [Region_Obj, setObj] = React.useState([
    new Region(
      -1,
      "default_poly",
      [0],
      ...
      .then((json) => {
        ...
        while (i < json.result.total) {
          ...
          while (keyArray.some((ele) => ele.includes(String(lastestYear)))) {
            ...
            temp_assault.push(
              json.result.records[i]["ASSAULT_" + String(lastestYear)]
            );
            temp_autotheft.push(
              json.result.records[i]["AUTOTHEFT_" + String(lastestYear)]
            );
            temp_biketheft.push(
              json.result.records[i]["BIKETHEFT_" + String(lastestYear)]
            );
            temp_breakenter.push(
              json.result.records[i]["BREAKENTER_" + String(lastestYear)]
            );
            temp_homicide.push(
              json.result.records[i]["HOMICIDE_" + String(lastestYear)]
            );
            temp_robbery.push(
              json.result.records[i]["ROBBERY_" + String(lastestYear)]
            );
            temp_shooting.push(
              json.result.records[i]["SHOOTING_" + String(lastestYear)]
            );
            temp_theftfrommv.push(
              json.result.records[i]["THEFTFROMMV_" + String(lastestYear)]
            );
            temp_theftover.push(
              json.result.records[i]["THEFTOVER_" + String(lastestYear)]
            );
          ...
          // console.log("center")
          // console.log(p)
          var temp_obj = new Region(
            json.result.records[i]._id,
            ...
            polylabel(
              JSON.parse(json.result.records[i].geometry).coordinates,
              0.3
            )
          );
  ...
  const Get_SD = (Region_Obj, option, avg) => {
    ...
    while (i < Region_Obj.length) {
      temp_sum += Math.pow(
        Region_Obj[i][option][year_option.indexOf(yearSelected)] - avg,
        2
      );
      i++;
    }
    ...
    while (i < Region_Obj.length) {
      ...
      if (crime_filter != "None") {
        if (Region_Obj[i][option][year_option.indexOf(yearSelected)] < z2) {
          ...
          Region_Obj[i][option][year_option.indexOf(yearSelected)] < z3
        ) {
          Region_Obj[i].region_color = region_color[1];
        } else if (
          Region_Obj[i][option][year_option.indexOf(yearSelected)] < z4
        ) {
          Region_Obj[i].region_color = region_color[2];
        } else if (
          Region_Obj[i][option][year_option.indexOf(yearSelected)] < z5
        ) {
          Region_Obj[i].region_color = region_color[3];
        } else if (
          Region_Obj[i][option][year_option.indexOf(yearSelected)] < z6
        ) {
          Region_Obj[i].region_color = region_color[3];
        } else if (
          Region_Obj[i][option][year_option.indexOf(yearSelected)] < z7
        ) {
      ...
      setObj(temp_front_regList.concat(Region_Obj[i], temp_end_regList));
      i++;
    }
  };
  ...
  const searchRegion = (searchName) => {
    ...
    Region_Obj.map((ele) => {
      if (ele.region_name == searchName) {
        ...
        var temp_count = ele[
          crime_option_value[crime_option.indexOf(crime_filter)]
        ][year_option.indexOf(yearSelected)]
          ? ele[crime_option_value[crime_option.indexOf(crime_filter)]][
              year_option.indexOf(yearSelected)
            ]
          : "No record";
        setBottomSheet([
  ...
  RegionPressed = (region) => {
    ...
    var temp_count = region[
      crime_option_value[crime_option.indexOf(crime_filter)]
    ][year_option.indexOf(yearSelected)]
      ? region[crime_option_value[crime_option.indexOf(crime_filter)]][
          year_option.indexOf(yearSelected)
        ]
      : "No record";
    setBottomSheet([
  ...
  const Marker_list = Region_Obj.map((item) => (
    ...
    >
      <Text>
        {
          (item[crime_option_value[crime_option.indexOf(crime_filter)]]
            ? item[crime_option_value[crime_option.indexOf(crime_filter)]]
            : ["0"])[year_option.indexOf(yearSelected)]
        }
      </Text>
    </Marker>
  ));
  ...
  const Picker_Option_Categlory = crime_option.map((item) => (
    <Picker.Item
      label={item}
      value={crime_option_value[crime_option.indexOf(item)]}
    />
  ));
  ...
  return (
    <View style={{ width: "100%", height: "100%", backgroundColor: "red" }}>
      ...
      >
        ...
        >
          <Text style={{ flex: 0.2, textAlign: "center" }}>
            {Math.round((avg - 2 * sd) * 10) / 10}
          </Text>
          <Text style={{ flex: 0.2, textAlign: "center" }}>
            {Math.round((avg - sd) * 10) / 10}
          </Text>
          <Text style={{ flex: 0.2, textAlign: "center" }}>
      ...
      <BottomSheet
        modalProps={{}}
        isVisible={bottom_sheet_visible}
        onBackdropPress={() => setBottomVisble(false)}
      >
